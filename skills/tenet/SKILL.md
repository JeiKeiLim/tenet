...
- Prefer `tenet_retry_job(job_id, enhanced_prompt)` for failed implementation jobs — it dispatches the retry immediately (retry = run-now), so apply any backoff BEFORE calling it, then wait with `tenet_job_wait`; do not call `tenet_start_job` on the retried job.
...
- `tenet_retry_job` dispatches the retried job immediately (status `running`) — it will not be returned by `tenet_continue()` as `next_job`, and `tenet_start_job` on it throws.
