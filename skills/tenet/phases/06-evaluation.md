...
| `contention` | Failure looks like sibling eval stepping on shared state | Re-run after siblings complete; if it recurs in parallel mode, re-run `tenet_validate_readiness`, wait for it to complete, then re-run the eval; if the re-run returns `passed: false` or omits the verdict, or contention still recurs, report it to the user (retryable from report scope; see 05-execution-loop.md) |
