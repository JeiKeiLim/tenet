...
| `contention` | Failure looks like sibling eval stepping on shared state | Re-run after siblings complete; if it recurs in parallel mode, re-run `tenet_validate_readiness` to re-evaluate the verdict, and if it still recurs after that, report it to the user (retryable from report scope) |
