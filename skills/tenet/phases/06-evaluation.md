...
| `contention` | Failure looks like sibling eval stepping on shared state | Re-run after siblings complete; if it recurs in parallel mode, re-run `tenet_validate_readiness` to re-evaluate the verdict, else report to the user (retryable from report scope) |
